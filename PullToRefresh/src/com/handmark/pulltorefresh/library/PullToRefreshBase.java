...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public static final int SMOOTH_SCROLL_DURATION_MS = 400;//200;
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 700;// 325;
    ...
    private boolean mHideHeaderLayout = false;
    private boolean mHideFooterLayout = false;
    ...
    private OnScrollBaseListener<T> mOnScrollListener;
    private OnRefreshListener<T> mOnRefreshListener;
    private OnRefreshListener2<T> mOnRefreshListener2;
    private OnPullEventListener<T> mOnPullEventListener;

    private SmoothScrollRunnable mCurrentSmoothScrollRunnable;
    protected boolean showLoadingHeader = true;
    protected boolean showLoadingFooter = true;
    protected boolean refreshableHasHeader = false;
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        if (refreshableView instanceof ViewGroup) {
            Log.d(LOG_TAG,(refreshableView instanceof AdapterView) + "," + (refreshableView instanceof ScrollView));
    ...
    public void setRefreshableHasHeader(boolean has){
        refreshableHasHeader = has;
        return;
    }
...
//        Log.d(LOG_TAG + "INTERCEPT",isPullToRefreshEnabled() + "");
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d(LOG_TAG + "TOUCH", isPullToRefreshEnabled() + "");
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    mIsBeingDragged = false;
                    Log.d(LOG_TAG, "ACTION_UP" + mState + "/" + (null != mOnRefreshListener) + "/" + (null != mOnRefreshListener2));
...
//        if(showLoadingHeader&& )
    ...
    public final void setOnScrollListener(OnScrollBaseListener<T> listener) {
    ...
    public void setForceRefreshing(Mode mode) {
        if (isRefreshing()) return;
        int w = View.MeasureSpec.makeMeasureSpec(0,
                View.MeasureSpec.UNSPECIFIED);
        int h = View.MeasureSpec.makeMeasureSpec(0,
                View.MeasureSpec.UNSPECIFIED);
        mHeaderLayout.measure(w, h);
        if(forceHeight == -1) forceHeight = mHeaderLayout.getMeasuredHeight();
        Log.d(LOG_TAG,forceHeight + "HHHHHHHHHHHHHHHHHH");
        switch (mode) {
            case PULL_FROM_START:
                if (mMode.showHeaderLoadingLayout()/* && showLoadingHeader*/) {
            ...
            case PULL_FROM_END:
            default:
                if (mMode.showFooterLoadingLayout()/*&& showLoadingFooter*/) {
        ...
        smoothScrollTo(mode == Mode.PULL_FROM_START ? -forceHeight : forceHeight);
    }
    ...
    protected final int getFooterSize() {
        return mHideFooterLayout ? 0 : mFooterLayout.getContentSize();
    }
    ...
    protected final int getHeaderSize() {
        return mHideHeaderLayout ? 0 : mHeaderLayout.getContentSize();
    }
    ...
    protected void onRefreshing(final boolean doScroll) {
        ...
        Log.d(LOG_TAG, doScroll + "/" + mShowViewWhileRefreshing + "/mode" + mCurrentMode);
        if (doScroll) {
            if (mShowViewWhileRefreshing) {
                ...
                Log.d(LOG_TAG, "mode:" + mCurrentMode);
                switch (mCurrentMode) {
                    ...
                    default:
                    case PULL_FROM_START:
                        Log.d(LOG_TAG, "hiding:" + mHideHeaderLayout + "/listener:" + (listener == null));
...
//            smoothScrollTo(0);
    ...
    /*
    * set mHideHeaderLayout/mFooterLayout to hide HeaderLayout/FooterLayout
    *
     */
    public final void setHideHeaderLayout(boolean mHideHeaderLayout) {
        this.mHideHeaderLayout = mHideHeaderLayout;
    }

    public final void setHideFooterLayout(boolean mHideFooterLayout) {
        this.mHideFooterLayout = mHideFooterLayout;
    }
    ...
    protected final void setHeaderScroll(int value) {
        if (DEBUG) {
            ...
            if (mOnScrollListener != null) mOnScrollListener.onScroll(this, value);
        }
        ...
        if (mLayoutVisibilityChangesEnabled) {
            if (value < 0) {
//                mHeaderLayout.setVisibility(showLoadingHeader ?View.VISIBLE:View.INVISIBLE);
                mHeaderLayout.setVisibility(View.VISIBLE);
            } else if (value > 0) {
//                mFooterLayout.setVisibility(showLoadingFooter ?View.VISIBLE:View.INVISIBLE);
    ...
    private void callRefreshListener() {
        if (null != mOnRefreshListener) {
            ...
            if (mCurrentMode == Mode.PULL_FROM_START /*&& showLoadingHeader*/) {
                mOnRefreshListener2.onPullDownToRefresh(this);
            } else if (mCurrentMode == Mode.PULL_FROM_END /*&& showLoadingFooter*/) {
                mOnRefreshListener2.onPullUpToRefresh(this);
            }
        }
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrLoadingHeader)) {
            showLoadingHeader = a.getBoolean(R.styleable.PullToRefresh_ptrLoadingHeader, true);

        }
        if (a.hasValue(R.styleable.PullToRefresh_ptrLoadingFooter)) {
            showLoadingFooter = a.getBoolean(R.styleable.PullToRefresh_ptrLoadingFooter, true);
        }
...
//            mHeaderLayout.setPullLabel("");
//            mHeaderLayout.setRefreshingLabel("");
//            mHeaderLayout.setReleaseLabel("");
//            mHeaderLayout.setLoadingDrawable(getResources().getDrawable(R.color.trans));
//        }
//        if (!showLoadingFooter) {
//            mFooterLayout.setLastUpdatedLabel("");
//            mFooterLayout.setPullLabel("");
//            mFooterLayout.setRefreshingLabel("");
//            mFooterLayout.setReleaseLabel("");
//            mFooterLayout.setLoadingDrawable(getResources().getDrawable(R.color.trans));
//        }
        /**
         * Styleables from XML
         */
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            switch (mCurrentMode) {
                case PULL_FROM_END:
                    hiding = mHideFooterLayout;
                    break;
                case PULL_FROM_START:
                default:
                    hiding = mHideHeaderLayout;
                    break;
            }
            if (!hiding) {
            ...
            Log.d(LOG_TAG, itemDimension + "," + Math.abs(newScrollValue) + "," + newScrollValue + "," + mState);
            if (mState != State.PULL_TO_REFRESH && (mState != State.RELEASE_TO_REFRESH || !hiding) && (itemDimension >= Math.abs(newScrollValue) || hiding)) {
    ...
    private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
        ...
        Log.d(LOG_TAG, oldScrollValue + "/" + newScrollValue);

        if (oldScrollValue != newScrollValue) {
            if (null == mScrollAnimationInterpolator) {
                // Default interpolator is a Decelerate Interpolator
                mScrollAnimationInterpolator = new DecelerateInterpolator(1f);
            }
    ...
    private final void smoothScrollToAndBack(int y) {
        smoothScrollTo(y, SMOOTH_SCROLL_DURATION_MS, 0, new OnSmoothScrollFinishedListener() {
            ...
            @Override
            public void onSmoothScrollFinished() {
                smoothScrollTo(0, SMOOTH_SCROLL_DURATION_MS / 2, DEMO_SCROLL_INTERVAL, null);
            }
        });
    }
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                ...
                case BANGYOUNG:
                    return new BangyoungLoadingLayout(context,mode,scrollDirection,attrs);
                case CATCH:
                default:
                    return new CatchLoadingLayout(context,mode,scrollDirection,attrs);
            }
        }
    }
    ...
    public static interface OnScrollBaseListener<V extends View> {
        /**
         * @param refreshView
         * @param scrollValue
         */
        public void onScroll(final PullToRefreshBase<V> refreshView, final int scrollValue);
    }
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
             ...
             */
            Log.d(LOG_TAG, "time:" + mStartTime + "/fromY:" + mScrollFromY + "/toY:" + mScrollToY);
            if (mStartTime == -1) {
            ...
            Log.d(LOG_TAG, mScrollToY + "," + mCurrentY + "," + mContinueRunning + ",listener:" + (null != mListener) + ",headerSize:" + getHeaderSize());
            if (mContinueRunning && mScrollToY != mCurrentY) {
                ViewCompat.postOnAnimation(PullToRefreshBase.this, this);
            } else {
                Log.d(LOG_TAG, "not triggered");
                if (null != mListener) {
